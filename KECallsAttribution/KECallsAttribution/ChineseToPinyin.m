...
+ (NSString *) pinyinFromChiniseString:(NSString *)string {
    ...
    for (int i = 0; i < iLen; i++) {
        if ((unsigned char)gb2312_string[i] < 0x80 ) {
			strValue = [[strValue stringByAppendingFormat:@"%c", gb2312_string[i] > 95 ? gb2312_string[i] - 32 : gb2312_string[i]]capitalizedString];
            continue;
		}
		...
		NSString *strRes = [FindLetter(nCode) capitalizedString];
